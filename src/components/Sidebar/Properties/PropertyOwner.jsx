...
function PropertyOwner({
  ...
  const subowners = useSelector((state) => {
    ...
    if (expansionIdentifier.includes("Software Integration") && subownersRaw.length) {
      const identifier = typeof subownersRaw == 'string' ? (subownersRaw[0].split(".")?.[1] ?? '') : ''
      shownSubowners.unshift(`${identifier}.removesoftwareintegrationsgn`)
    }
  ...
  return !isHidden && (
    ...
    >
      ...
      { subowners.map((ownerUri) => {
        ...
        if (ownerUri.includes("removesoftwareintegrationsgn") && splitUri?.[0]) {
          return <RemoveSoftwareIntegrationSgn key={ownerUri} identifier={splitUri[0]} />
        }
